...
export default function EditarProducto() {
  ...
  const validateForm = () => {
    ...
    } else if (productData.nombre.length < 2) {
    ...
    // Validar cantidad
    if (!productData.cantidad || productData.cantidad.trim() === "") {
    ...
    // Validar sexo
    if (!productData.idsexo || productData.idsexo === 0) {
      newErrors.idsexo = "Debe seleccionar un sexo";
    }

    // Validar marca
    if (!productData.idmarca || productData.idmarca === 0) {
      newErrors.idmarca = "Debe seleccionar una marca";
    }

    // Validar categoría
    if (!productData.idcategoria || productData.idcategoria === 0) {
  ...
  useEffect(() => {
    if (producto) {
      setProductData({
        ...
        descripcion: producto.attributes.descripcion || "",
        volumen: producto.attributes.volumen || "",
        idmarca: producto.attributes.marca || 0,
        idsexo: producto.attributes.sexo || 0,
        idcategoria: producto.attributes.idcategoria || 0,
      });
      ...
      if (producto.attributes.imagen == null) {
        ...
        const imageUrl = `http://${dirImg}${producto.attributes.imagen.split("/").pop()}`;
  ...
  const obtenerSexosyMarcas = async () => {
    try {
      const [Csexos, Cmarcas, Ccategorias] = await Promise.all([
        api.get("/sexo"),
        api.get("/marcas"),
        api.get("/categorias")
      ]);
  ...
  const updateProduct = async () => {
    ...
    try {
      ...
      const dataToSend = {
        ...
        volumen: productData.volumen.trim(),
        idsexo: parseInt(productData.idsexo),
        idmarca: parseInt(productData.idmarca),
        idcategoria: parseInt(productData.idcategoria),
        imagen_id: idimagen,
      };

      // Validar que los números sean válidos
      if (isNaN(dataToSend.precio) || isNaN(dataToSend.cantidad) || 
          isNaN(dataToSend.idsexo) || isNaN(dataToSend.idmarca) || 
          isNaN(dataToSend.idcategoria)) {
    ...
    >
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        <View style={styles.formContainer}>
          ...
          <View style={[styles.pickerContainer, errors.idsexo && styles.pickerError]}>
            <RNPickerSelect
              key={`sexo-picker-${pickerKey}`}
              placeholder={{ label: "Selecciona un sexo", value: null }}
              onValueChange={(value) => handleChange("idsexo", value)}
              ...
              fixAndroidTouchableBug={true}
            />
            {errors.idsexo && <Text style={styles.errorText}>{errors.idsexo}</Text>}
          </View>

          <View style={[styles.pickerContainer, errors.idmarca && styles.pickerError]}>
            <RNPickerSelect
              key={`marca-picker-${pickerKey}`}
              placeholder={{ label: "Selecciona una marca", value: null }}
              onValueChange={(value) => handleChange("idmarca", value)}
              ...
              fixAndroidTouchableBug={true}
            />
            {errors.idmarca && <Text style={styles.errorText}>{errors.idmarca}</Text>}
          </View>

          <View style={[styles.pickerContainer, errors.idcategoria && styles.pickerError]}>
            <RNPickerSelect
              key={`categoria-picker-${pickerKey}`}
              placeholder={{ label: "Selecciona una categoría", value: null }}
              onValueChange={(value) => handleChange("idcategoria", value)}
              items={categorias.map((categoria) => ({
                label: categoria.attributes?.descripcion || "Sin descripción",
              ...
              fixAndroidTouchableBug={true}
            />
            {errors.idcategoria && <Text style={styles.errorText}>{errors.idcategoria}</Text>}
          </View>
        </View>
